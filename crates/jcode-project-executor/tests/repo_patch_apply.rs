...
#[tokio::test]
async fn repo_patch_apply_rejects_ready_session_without_current_custody() {
    let (root, first, execution_id, repository, workspace) = managed_session().await;
    let base_sha = git(std::path::Path::new(&workspace), &["rev-parse", "HEAD"]);
    let contender = ProjectExecutorSupervisor::create_with_worktree_root_and_device_id(
        root.path().join("sessions"),
        root.path(),
        "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
        "11111111-1111-4111-8111-111111111111",
    ...
    .unwrap();
    let rejected = handle_executor_request(
        &contender,
        patch_request(&execution_id, &repository, &base_sha),
    )
    .await;
    assert!(!rejected.ok);
    let error = rejected.error.unwrap_or_default();
    assert!(
        error.contains("runnable READY execution") || error.contains("CUSTODY_NOT_CURRENT"),
