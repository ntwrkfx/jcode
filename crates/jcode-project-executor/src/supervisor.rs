...
impl ProjectExecutorSupervisor {
    ...
    pub async fn apply_repository_patch(
        ...
        if inspection.lifecycle != SessionLifecycle::Ready
            || inspection.runnability != SessionRunnability::Runnable
        {
            bail!("repository patch requires a runnable READY execution: {execution_id}");
        }
        ...
        // Material mutation is fenced by the current provider-issued custody
        // generation at the effect boundary; a stale or missing grant fails closed.
        let grant = self
            .custody_grants
            .lock()
            .await
        ...
        let evidence = serde_json::json!({
            ...
            "custody_generation": grant.generation,
        });
